...
def calc_mean_std(tensor: torch.Tensor, eps=1e-5):
    mean = tensor.mean(dim=[2, 3], keepdim=True)
    std = (tensor.var(dim=[2, 3], keepdim=True) + eps).sqrt()
    return mean, std


def normalize(tensor: torch.Tensor, eps=1e-5):
...
class StyleTransfer(nn.Module):
    def __init__(self, fusion, encoder, decoder):
        super().__init__()
        self.fusion = fusion
    ...
    def encode_with_intermediate(self, tensor):
        results = [tensor]
        for i in range(5):
    ...
    def calc_content_loss_once(self, pred, target):
        assert (pred.size() == target.size())
        # assert (target.requires_grad is False)
        return self.mse_loss(pred, target)

    def calc_content_loss(self, preds, targets):
    ...
    def calc_style_loss_once(self, pred, target):
        assert (pred.size() == target.size())
    ...
    def calc_style_loss(self, preds, targets):
        assert len(preds) == len(targets)
